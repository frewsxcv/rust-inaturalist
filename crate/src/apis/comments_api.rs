...
pub struct CommentsIdPutParams {
    ...
    pub id: i32,
    /// Comment object
    pub body: Option<models::PostComment>,
}
...
#[serde(untagged)]
pub enum CommentsIdDeleteError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`comments_id_put`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CommentsIdPutError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`comments_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CommentsPostError {
    UnknownValue(serde_json::Value),
}

/// Delete a comment
pub async fn comments_id_delete(
    configuration: &configuration::Configuration,
    params: CommentsIdDeleteParams,
) -> Result<(), Error<CommentsIdDeleteError>> {
    let uri_str = format!("{}/comments/{id}", configuration.base_path, id = params.id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<CommentsIdDeleteError> = serde_json::from_str(&content).ok();
...
/// Update a comment
pub async fn comments_id_put(
    configuration: &configuration::Configuration,
    params: CommentsIdPutParams,
) -> Result<(), Error<CommentsIdPutError>> {
    let uri_str = format!("{}/comments/{id}", configuration.base_path, id = params.id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CommentsIdPutError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn comments_post(
    configuration: &configuration::Configuration,
    params: CommentsPostParams,
) -> Result<(), Error<CommentsPostError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CommentsPostError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
