...
pub struct ObservationFieldValuesIdPutParams {
    ...
    pub body: Option<models::PostObservationFieldValue>,
}
...
#[serde(untagged)]
pub enum ObservationFieldValuesIdDeleteError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`observation_field_values_id_put`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ObservationFieldValuesIdPutError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`observation_field_values_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ObservationFieldValuesPostError {
    UnknownValue(serde_json::Value),
}

/// Delete an observation field value
pub async fn observation_field_values_id_delete(
    configuration: &configuration::Configuration,
    params: ObservationFieldValuesIdDeleteParams,
) -> Result<(), Error<ObservationFieldValuesIdDeleteError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<ObservationFieldValuesIdDeleteError> =
...
pub async fn observation_field_values_id_put(
    configuration: &configuration::Configuration,
    params: ObservationFieldValuesIdPutParams,
) -> Result<(), Error<ObservationFieldValuesIdPutError>> {
    let uri_str = format!(
        "{}/observation_field_values/{id}",
        configuration.base_path,
        id = params.id
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ObservationFieldValuesIdPutError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn observation_field_values_post(
    configuration: &configuration::Configuration,
    params: ObservationFieldValuesPostParams,
) -> Result<(), Error<ObservationFieldValuesPostError>> {
    let uri_str = format!("{}/observation_field_values", configuration.base_path);
    let mut req_builder = configuration
        .client
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ObservationFieldValuesPostError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
