...
pub struct FlagsIdPutParams {
    /// ID of the record
    pub id: i32,
    pub body: Option<models::PutFlag>,
}
...
pub struct FlagsPostParams {
    /// Flag object
    pub body: Option<models::PostFlag>,
}

/// struct for typed errors of method [`flags_id_delete`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum FlagsIdDeleteError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`flags_id_put`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum FlagsIdPutError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`flags_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum FlagsPostError {
    UnknownValue(serde_json::Value),
}

/// Delete a flag
pub async fn flags_id_delete(
    configuration: &configuration::Configuration,
    params: FlagsIdDeleteParams,
) -> Result<(), Error<FlagsIdDeleteError>> {
    let uri_str = format!("{}/flags/{id}", configuration.base_path, id = params.id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<FlagsIdDeleteError> = serde_json::from_str(&content).ok();
...
pub async fn flags_id_put(
    configuration: &configuration::Configuration,
    params: FlagsIdPutParams,
) -> Result<(), Error<FlagsIdPutError>> {
    let uri_str = format!("{}/flags/{id}", configuration.base_path, id = params.id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<FlagsIdPutError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn flags_post(
    configuration: &configuration::Configuration,
    params: FlagsPostParams,
) -> Result<(), Error<FlagsPostError>> {
    let uri_str = format!("{}/flags", configuration.base_path);
    let mut req_builder = configuration
        .client
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<FlagsPostError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
