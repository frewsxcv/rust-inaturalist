...
#[serde(untagged)]
pub enum ControlledTermsForTaxonGetError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`controlled_terms_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ControlledTermsGetError {
    UnknownValue(serde_json::Value),
}

/// Returns attribute controlled terms relevant to a taxon
pub async fn controlled_terms_for_taxon_get(
    configuration: &configuration::Configuration,
    params: ControlledTermsForTaxonGetParams,
) -> Result<(), Error<ControlledTermsForTaxonGetError>> {
    let uri_str = format!("{}/controlled_terms/for_taxon", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<ControlledTermsForTaxonGetError> = serde_json::from_str(&content).ok();
...
pub async fn controlled_terms_get(
    configuration: &configuration::Configuration,
) -> Result<(), Error<ControlledTermsGetError>> {
    let uri_str = format!("{}/controlled_terms", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ControlledTermsGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
